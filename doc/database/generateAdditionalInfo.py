...
def main(result_list):
    ...
    text = ""
    spacing = "        "
    for name, info in result_list.items():
    
    # name = "Kirlia"
    # info = result_list[name]
        
        tree = generate_evolution_tree(info["evolutions"], result_list)
...
def generate_evolution_tree(evolution_chain, result_list):
    # Special cases:
    ...
    if result_list.has_key(evolution_chain[2]):
        # 3rd element is a name
        return EvolutionNode(name, {evolution_chain[1]: generate_evolution_tree(evolution_chain[2:], result_list)})
    else:
        # 3rd element is a path
        evolution_paths = filter(lambda x: not result_list.has_key(x), evolution_chain[1:])
        evolutions = map(lambda x: EvolutionNode(x, {}), filter(lambda x: result_list.has_key(x), evolution_chain[1:]))
