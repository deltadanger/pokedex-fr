...
def main(types, megatypes, caracs, datatalents):
    ...
    for name, p in list.items():
        ...
        for talent in p["talents"]:
            text += "this.add(Ability.{});".format(talent)
        
        p["name"] = toId(p["name"])
        
        text += """}}}};
        perName.put(ctx.getString(R.string.name_{name}), new Pokemon(ctx.getString(R.string.name_{name}), {number}, {index}, Type.{type1}, Type.{type2}, abilities, {life}, {attack}, {defense}, {spattack}, {spdefense}, {speed}));""".format(**p)
    print text
